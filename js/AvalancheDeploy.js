...
(function (b, a) {
    a.Lightboxer = function (c) {
        ...
        b.extend(true, e.lightbox_meOptions, {
            classPrefix: e.skin,
            horizontallyCentered: false,
            closeClick: false,
            centered: true,
            beforeClose: function () {
                if (d.currentItem && d.currentItem.player && d.currentItem.player.destroy) {
                    d.currentItem.player.destroy()
                }
                d.currentItem = null;
                if (e.linkScanning) {
                    b(e.target).removeClass(e.selectedClass)
                }
            },
            onClose: function () {
                if (NYTD.NYTMM.iOS && NYTD.NYTMM.iOSInfo.version >= 6) {
                    b("body").off("touchend click.nytmm_lightboxer", "#nytmm_lb_nav, #nytmm_lb_all .nytmm_lb_close_hitbox")
                } else {
                    b("#nytmm_lb_nav, #nytmm_lb_all .nytmm_lb_close_hitbox").unbind(".nytmm_lightboxer")
                }
            },
            onLoad: function () {
                if (e.linkScanning) {
                    b(e.target).removeClass(e.selectedClass).filter(function (g) {
                        return (e.getID(d.extractData(this), g) === e.getID(d.currentItem, d.index()))
                    }).addClass(e.selectedClass).end()
                }
                var f = b("#nytmm_lightboxer .nytmm_lb_slideshowBox").data("SliderShow");
                if (f) {
                    f.autosizeCaptions()
                }
                d.checkSize();
                if (NYTD.NYTMM.iOS) {
                    if (NYTD.NYTMM.iOSInfo.version >= 6) {
                        b("body").on("touchstart click.nytmm_lightboxer", "#nytmm_lb_nav, #nytmm_lb_all .nytmm_lb_close_hitbox", function (g) {
                            d.close();
                            g.preventDefault();
                            return false
                        })
                    } else {
                        b("#nytmm_lb_nav, #nytmm_lb_all .nytmm_lb_close_hitbox").bind("click.nytmm_lightboxer", function () {
                            d.close()
                        })
                    }
                }
                e.onOpen(d.currentItem, d.index());
                b(document).trigger(a.Lightboxer.OPEN_EVENT, d.currentItem, d.index())
            }
        });
        b(document).on("click.nytmm_lightboxer", ".nytmm_lb_next", function (f) {
            d.next(e.wrap);
            b.log("NEXT", d._index);
            f.preventDefault();
            return false
        });
        b(document).on("click.nytmm_lightboxer", ".nytmm_lb_previous", function (f) {
            d.previous(e.wrap);
            b.log("PREV", d._index);
            f.preventDefault();
            return false
        ...
        if (this.options.swipe) {
            b(document).swipe({
                threshold: 75,
                maxTimeThreshold: 1600,
                disableDesktopSwipeWebKit: true,
                swipeLeft: function (f) {
                    d.next(e.wrap);
                    f.preventDefault();
                    return false
                },
                swipeRight: function (f) {
                    d.previous(e.wrap);
                    f.preventDefault();
                    return false
                },
        ...
        if (!NYTD.NYTMM.iOS) {
            b(document).on("click.nytmm_lightboxer", "#nytmm_lb_nav, #nytmm_lb_all .nytmm_lb_close", function () {
                d.close()
            })
        }
        b(document).on("click.nytmm_lightboxer", e.target, function (g) {
            var f = d.extractData(this);
            d.open(f);
            g.preventDefault();
            return false
        });
        if (this.options.keyboard) {
            b(this.options.keyboard).on("keydown.nytmm_lightboxer", function (g) {
                if (g.target.nodeName === "INPUT" || g.target.nodeName === "TEXTAREA") {
                    return true
                }
                var f = true;
                switch (g.keyCode) {
        ...
        if (this.options.autoAdvance) {
            b(document).on(a.VideoEvents.END_EVENT, "#nytmm_lightboxer", function () {
                b.log("END EVENT");
                d.next(e.wrap)
            })
        }
        if (this.options.linkScanning) {
            this.scan()
        }
        return this
    };
